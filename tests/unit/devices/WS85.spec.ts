...
configs.forEach(config => {
  describe(`WS85 device should be configured ${config}`, () => {
    ...
    it('Services are created with name overrides', (done) => {
      ...
      platform.config.nameOverrides[7] = {'key': 'WS85:rainLast24hTotal', 'value': 'Test Rain Last 24h Total Name'};
      platform.config.nameOverrides[8] = {'key': 'WS85:rainDailyTotal', 'value': 'Test Rain Daily Total Name'};
      platform.config.nameOverrides[9] = {'key': 'WS85:rainWeeklyTotal', 'value': 'Test Rain Weekly Total Name'};
      platform.config.nameOverrides[10] = {'key': 'WS85:rainMonthlyTotal', 'value': 'Test Rain Monthly Total Name'};
      platform.config.nameOverrides[11] = {'key': 'WS85:rainYearlyTotal', 'value': 'Test Rain Yearly Total Name'};
    ...
    it('Services are not created when hidden with device-specific override', (done) => {
      ...
      platform.config.hidden['WS85:rainLast24hTotal'] = true;
