...
import { GW3000 } from './../../../src/devices/GW3000';
...
configs.forEach(config => {
  describe(`GW3000 device should be configured ${config}`, () => {
    before('Initialize platform', () => {
      platform = createPlatform(config);
      accessory = new api.platformAccessory('Accessory', "5746853e-4fee-4e47-97dd-53065ef1de03")
      device = new GW3000(platform, accessory, "GW3000");
    });
    ...
    it('Services are created with name overrides', (done) => {
      platform.config.nameOverrides[0] = {"key": "GW3000:indoorTemperature", "value": "Test Temperature Name"};
      platform.config.nameOverrides[1] = {"key": "GW3000:indoorHumidity", "value": "Test Humidity Name"};
    ...
    it('Services are not created when hidden with device-specific override', (done) => {
      platform.config.hidden["GW3000:indoorTemperature"] = true;
      platform.config.hidden["GW3000:indoorHumidity"] = true;
