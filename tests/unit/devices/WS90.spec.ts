...
configs.forEach(config => {
  describe(`WS90 device should be configured ${config}`, () => {
    ...
    it('Services are created with name overrides', (done) => {
      ...
      platform.config.nameOverrides[11] = {'key': 'WS90:rainLast24hTotal', 'value': 'Test Rain Last 24h Total Name'};
      platform.config.nameOverrides[12] = {'key': 'WS90:rainDailyTotal', 'value': 'Test Rain Daily Total Name'};
      platform.config.nameOverrides[13] = {'key': 'WS90:rainWeeklyTotal', 'value': 'Test Rain Weekly Total Name'};
      platform.config.nameOverrides[14] = {'key': 'WS90:rainMonthlyTotal', 'value': 'Test Rain Monthly Total Name'};
      platform.config.nameOverrides[15] = {'key': 'WS90:rainYearlyTotal', 'value': 'Test Rain Yearly Total Name'};
    ...
    it('Services are not created when hidden with device-specific override', (done) => {
      ...
      platform.config.hidden['WS90:rainLast24hTotal'] = true;
