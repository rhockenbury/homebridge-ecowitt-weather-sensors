...
export class WH52 extends EcowittAccessory {
  ...
  constructor(
    ...
    super(platform, accessory, 'WH52', 'WH52 3-in-1 Soil Sensor', channel);

    this.requiredData = [`soil_ec_batt${this.channel}`, `soil_ec_hum${this.channel}`,
      `soil_ec_temp${this.channel}`, `soil_ec${this.channel}`];
    ...
    if (!utils.includesAny(hidden, ['soiltemperature', `${this.shortServiceId}:soiltemperature`])) {
      ...
      this.soilMoisture = new TemperatureSensor(platform, accessory, `${this.accessoryId}:soiltemperature`, nameOverride || 'Soil Temperature');
    } else {
      this.soilMoisture = new TemperatureSensor(platform, accessory, `${this.accessoryId}:soiltemperature`, 'Soil Temperature');
      this.soilMoisture.removeService();
      this.soilMoisture = undefined;
    }
    ...
    if (!utils.includesAny(hidden, ['soilconductivity', `${this.shortServiceId}:soilconductivity`])) {
      ...
      this.soilMoisture = new ConductivitySensor(platform, accessory, `${this.accessoryId}:soilconductivity`, nameOverride || 'Soil Conductivity');
    } else {
      this.soilMoisture = new ConductivitySensor(platform, accessory, `${this.accessoryId}:soilconductivity`, 'Soil Conductivity');
      this.soilMoisture.removeService();
      this.soilMoisture = undefined;
    }
  }
