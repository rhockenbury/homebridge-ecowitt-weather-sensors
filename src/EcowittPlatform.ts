...
export class EcowittPlatform implements DynamicPlatformPlugin {
  ...
  constructor(
    ...
    // custom thresholds format introduced in v2.11.0 require remapping thresholds
    updatedConfig = utils.thresholdsRemapper(updatedConfig);

    if (JSON.stringify(updatedConfig) === JSON.stringify(this.config)) {
      this.log.debug('Plugin config update is not required');
    } else {
