...
| additional.staticNames | `true` | Set to `false` to show the weather metric value in the service names of the accessory. For example, it will display `Temperature 73.40°F` rather than `Temperature`.  In some cases, this will generate warnings visible in the Homebridge logs that look like `... has an invalid 'ConfiguredName' characteristic ('Temperature 73.40°F'). Please use only alphanumeric, space, and apostrophe characters...` |
